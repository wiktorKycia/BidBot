...
def plot_all(stats: dict):
    ...
    kpis = [
        ...
        ("Plików łącznie\n(pobrane + z ZIPów)", f"{total_atts + stats['extracted_file_count']:,}"),
        ("Śr. załączników / oferta", f"{avg_atts:.1f}"),
        ("Śr. rozmiar pliku", fmt_size(avg_size)),
        ("ZIPy nierozpakowane", f"{stats['zip_not_extracted']}"),
        ("Podfoldery (ZIP extract)", f"{stats['subfolder_count']}"),
        ("Pliki z ZIPów", f"{stats['extracted_file_count']:,}"),
...
def main():
    ...
    total_files = sum(stats["att_counts"]) + stats["extracted_file_count"]
    print("\n" + "═" * 50)
    print(f"  Łączna liczba ofert:               {stats['total_offers']:>6}")
    print(f"  Plików łącznie (pobrane + z ZIPów): {total_files:>5}")
    print(f"    w tym pobrane bezpośrednio:       {sum(stats['att_counts']):>5}")
    print(f"    w tym z ZIPów (rekurencyjnie):    {stats['extracted_file_count']:>5}")
    print(f"  Średnio załączników/oferta:         {np.mean(stats['att_counts']):.2f}" if stats["att_counts"] else "  Brak danych")
    print(f"  ZIPy nierozpakowane:                {stats['zip_not_extracted']:>5}")
